...
######## CHANGED CG 15 May ###### START
sample_effect_size <- function() {
  if (runif(1) < 0.85) {
    runif(1, min = 0.01, max = 0.05)
  } else {
    runif(1, min = 0.1, max = 0.3)
  }
}
######## CHANGED CG 15 May ###### END
...
)
{
  ...
  for(i in 1:n.traits){
    ...
    causal_snps=row.names(genos$X)[causal_idx]
        ...
        ######## CHANGED CG 15 May ###### START
        block_size <- 50  # Adjustable: could pass as param
        trait_block <- ceiling(i / block_size)
        if (!exists("block_effect_sizes")) {
          n_blocks <- ceiling(n.traits / block_size)
          block_effect_sizes <- sapply(1:n_blocks, function(x) sample_effect_size())
        }
        this_effect_size <- block_effect_sizes[trait_block]
        betas=beta.calc(r2=this_effect_size, maf=freqs[causal_idx])
        ######## CHANGED CG 15 May ###### END
