...
setwd("/scratch/giulia.desanctis/susie_results_method_2/result_spreadsheets/10k_indiv_1_perc/")
...
sim_lookup <- df %>%
  filter(is_sim) %>%
  select(model_number, effect_size, n_phenos) %>%
  distinct()
...
df_sim <- df %>%
  ...
  select(model_name, model_number, n_phenos, effect_size, coverage, power)
...
summary_df <- df_combined %>%
  dplyr::group_by(model_name, effect_size, n_phenos) %>%
  dplyr::summarize(
...
p_cov <- ggplot(summary_df, aes(x = n_phenos, y = mean_coverage, color = model_name)) +
  ...
  facet_wrap(~ effect_size, scales = "free_y", labeller = label_both) +
  labs(
    x = "Number of Phenotypes",
    y = "Mean Coverage",
    color = "Method",
    title = "Coverage vs. Number of Phenotypes - 10k individuals & 1% phenotypes with effect "
  ) +
...
p_pow <- ggplot(summary_df, aes(x = n_phenos, y = mean_power, color = model_name)) +
  ...
  labs(
    ...
    y = "Mean Power",
    color = "Method",
    title = "Power vs. Number of Phenotypes - 10k individuals 1% phenotypes with effect"
  ) +
  theme_minimal(base_size = 14) +
  theme(
    strip.text = element_text(face = "bold", size = 14),
...
p_together <- ggplot(df_long, aes(x = n_phenos, y = Value, color = model_name, linetype = Metric)) +
  ...
  labs(
    title = "Fine-mapping performance with 10k individuals and 1% phenotypes with effect",
    x = "Number of Phenotypes",
    y = "Value",
    color = "Method",
    linetype = "Metric"
