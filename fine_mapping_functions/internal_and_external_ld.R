...
external_and_internal_ld <- function(
    ...
    ,individual.idxs.int=NULL
    ,num_indiv_subsample = 1000
) 
{
  setwd(sprintf("/scratch/giulia.desanctis/susie_results_method_2/%d_individuals/model_%d", num_indiv_subsample, model.no))
  
  # Internal LD 
  commando=paste(plink2.binary,"--bfile",bed.internal,"--keep-allele-order","--chr",chr,"--from-kb",start.pos,"--to-kb",end.pos,"--maf",maf.threshold,"--keep", individual.idxs.int, "--out ", b.file.extension.int," --make-bed")
  system(commando)
...
for(i in 1:10) {
  external_and_internal_ld(
    ...
    ,bed.external="/scratch/giulia.desanctis/british_data_split_for_int_ext/external_one_third"
    ,plink2.binary="/ssu/gassu/software/plink/2.00_20211217/plink2"
    ,plink.binary="/ssu/gassu/software/plink/1.90_20210606/plink"
    ,effect.size= c(0.005, 0.05, 0.2)
    ,maf.threshold=0.01
    ,chr=chromosomes[i]
    ...
    ,b.file.extension.int="all_indiv_int"
    ,b.file.extension.ext="all_indiv_ext"
    ,individual.idxs.int="~/individual_lists/individual_idxs_internal_10k.txt"
    ,num_indiv_subsample = 10000
  ) 
