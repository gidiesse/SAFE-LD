...
susie_launcher <- function (
  ...
  setwd(sprintf("/scratch/giulia.desanctis/susie_results/%d_individuals/model_%d", n_individuals_R_sim, model.no))
  ...
  withCallingHandlers({
    ...
    R_simulated <- readRDS(sprintf(file_path_R_sim, n_individuals_R_sim, R_sim.no, R_sim.no))
    ...
    sim_suffixes <- apply(sim_suffixes, 1, function(row) {
      paste0("simulated_LD_", row["ld"], "_phenos_", row["n"], "_individuals_more_phenos_than_individuals")
    })
    ...
    saveRDS(results_df, sprintf("susie_metrics_%d_more_phenos_than_indiv.rds", model_number))
...
susie_launcher(
  ...
  , file_path_R_sim = "/scratch/giulia.desanctis/susie_results/%d_individuals/model_%d/model_%d_with_R_sim_more_phenos_than_indiv.rds"
  , file_path_simulation = "/scratch/giulia.desanctis/susie_results/%d_individuals/model_%d/model_%d_with_R_int_R_ext_and_effects.rds"
  , max_iter_external_susie = 15000
  , max_iter_internal_susie = 2000
  , max_iter_simulated_susie = 8000
  , fixed_LDs = c("internal_LD", "external_LD")
  , sim_LDs = c(5000, 6000, 7000, 8000, 9000, 10000)
  , n_individuals_R_sim = 5000
)
